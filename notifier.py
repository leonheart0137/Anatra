# notifier.py
import os, platform, subprocess, threading, time, shutil, regex, requests
...
class Notifier:
    ...
    def __init__(self,
        ...
        self.gif_path = gif_path or os.path.join(os.path.dirname(__file__), "emojis", "magic.gif")
        self.intro_sound = intro_sound or os.path.join(os.path.dirname(__file__), "sound", "intro.wav")
        self.click_sound = click_sound or os.path.join(os.path.dirname(__file__), "sound", "pop.wav")
        self.pitch_sound = pitch_sound or os.path.join(os.path.dirname(__file__), "sound", "pitch.wav")
        self.beep = beep
    ...
    def show(self):
        ...
        msg_label = ctk.CTkLabel(self.frame, text=self.message,
                                 fg_color=self.BG, text_color=self.FG2,
                                 font=ctk.CTkFont(family="Segoe UI", size=14))
        msg_label.place(x=80, y=45)

        button_frame = ctk.CTkFrame(self.frame, fg_color=self.BORDER_COLOR,
                                    corner_radius=6, width=self.WIDTH-40, height=28)
        button_frame.place(relx=0.5, y=90, anchor="n")

        button_label = ctk.CTkLabel(button_frame,
